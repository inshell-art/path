...
sncast_declare_json_pkg() {
  local pkg="$1" cname="$2"
  sncast --account "$ACCOUNT" --accounts-file "$ACCOUNTS_FILE" --json declare --url "$RPC" --package "$pkg" --contract-name "$cname" | filter_json_lines
}
...
class_hash_from_pkg() {
  local pkg="$1" cname="$2"
  sncast utils class-hash --package "$pkg" --contract-name "$cname" | awk '/Class Hash:/ {print $3}' | tail -n1
}
