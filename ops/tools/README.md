...
- `postdeploy_smoke.py` is a Dev OS-only live buyer-flow smoke helper for completed Sepolia deploys. It serves a local Rabby/browser-wallet page that ABI-encodes `bid(uint256)` at runtime, simulates before sending, verifies mined calldata after sending, and writes local smoke evidence under `output/smoke/<network>/<run_id>/`.
...
- `export_signing_os_bundle.py` converts one immutable `bundles/<network>/<run_id>/` input into a portable `PATH-RUN-BUNDLE/`; it builds a sparse clean workspace at the bundle's pinned commit, vendors the local `evm/node_modules`, precompiles Hardhat artifacts on Dev OS, and emits operator-facing `bin/preflight-signingos`, `bin/verify`, `bin/approve`, `bin/apply`, and `bin/postconditions` wrappers that each write a top-level result dir under `results/`.
...
- `export_local_fe_release.mjs` exports the same frontend handoff shape from `evm/deployments/localhost-eth.json` for local Anvil/Hardhat rehearsal. It is devnet-only and validates on-chain code and deploy receipts before writing `artifacts/devnet/current/fe-release/`.
